...
export default function Step09({ onNext, onStepClick }: Step09Props) {
  const updateAnswer = useQuizStore(state => state.updateAnswer);

  const handleAnswer = (answer: string) => {
    updateAnswer('step9', answer);
    if (onNext) onNext();
  };
  return (
    <div className="relative w-full h-screen flex flex-col rounded-2xl bg-transparent box-border overflow-hidden text-center text-[42px] text-darkslategray font-geist">
          ...
          <div className="flex flex-col items-center gap-4 sm:gap-6 md:gap-8">
            ...
            <div className="flex flex-col items-center justify-center gap-3 sm:gap-4 text-sm sm:text-base text-white font-inter w-full">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-start gap-3 sm:gap-4 w-full">
                <button
                  onClick={() =>
                    handleAnswer('Yes, we need MIDs/payment processing')
                  }
      ...
      <ProgressIndicators currentStep={9} onStepClick={onStepClick} />
    </div>
  );
}
